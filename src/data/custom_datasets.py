...
class CrackTipDataset(Dataset):
    ...
    """
    def __init__(self, inputs, targets=None, transform=None):
        ...
        if self.data_paths_label[0] is not None:
            self.targets = torch.cat([out_tensor
                                      for i in [torch.load(out) for out in targets]
                                      for out_tensor in i]
                                     )
    ...
    def __getitem__(self, num):
        ...
        if self.data_paths_label[0] is not None:
            ...
            sample = {'input': image, 'target': target, 'tip': tip}
        else:
            sample = {'input': image}
...
class ExplCrackTipDataset(CrackTipDataset):

    def __init__(self, inputs, targets=None, transform=None, critic_ai=None):
        super().__init__(inputs, targets, transform)
        self.inputs.float()
        self.explanations = None

        if targets is not None:
            self.explanations = [np.expand_dims(critic_ai(x,y), axis=0) for x,y in self.crack_tips]
    ...
    def __getitem__(self, num):
        ...
        # check if we have a corresponding target
        if self.data_paths_label[0] is not None:
        ...
        if self.explanations:
            field = torch.from_numpy(self.explanations[num])
            sample['explanation'] = field            

        # check if we want to apply transforms
        if self.transform:
            sample = self.transform(sample)
